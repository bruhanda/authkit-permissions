/**
 * advanced-usage — realistic multi-tenant document-management scenario.
 *
 *   - role inheritance (`superadmin` > `admin` > `member` > `viewer`)
 *   - cross-tenant access gated by **two** flags (defence in depth)
 *   - sync ABAC condition with a filter hint that lowers into SQL
 *   - async ABAC condition (collaborator lookup)
 *   - structured audit hook with timing + JSON formatter
 *   - `accessibleBy()` lowering into Prisma / Mongo `where` clauses
 */
import {
  composeAudit,
  createEnforcer,
...
const policy = definePolicy({
  ...
  conditions: { isOwner, notArchived, isCollaborator },
  permissions: {
...
const alice = createSubject({ id: 'u_alice', roles: ['member'] as const, tenantId: 't_acme' });
const bob   = createSubject({ id: 'u_bob',   roles: ['admin']  as const, tenantId: 't_acme' });
const carol = createSubject({ id: 'u_carol', roles: ['member'] as const, tenantId: 't_acme' });
const root  = createSubject({ id: 'u_root',  roles: ['superadmin'] as const, tenantId: 't_authkit' });
...
async function main(): Promise<void> {
  ...
  console.log(
    ...
    await enforcer.check({ subject: alice, resource: 'document', action: 'update', data: docAlpha }),
  );
  console.log(
  ...
  console.log(
    ...
    await enforcer.check({ subject: carol, resource: 'document', action: 'update', data: docAlpha }),
  );

  console.log('\n=== explain() surfaces grantedBy + condition ===');
  console.log(
    await enforcer.explain({ subject: alice, resource: 'document', action: 'delete', data: docAlpha }),
  );
  ...
  const ast = enforcer.accessibleBy({ subject: alice, resource: 'document', action: 'delete' });
  ...
  console.log(`\n=== ${auditEvents.length} audit events captured for SOC 2 logs ===`);
}
