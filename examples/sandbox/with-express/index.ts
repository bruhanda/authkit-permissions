/**
 * with-express — Express adapter end-to-end.
 *
 * Wires `expressPermissions(...)` between a toy auth middleware and a
 * route handler. Exercises:
 *
...
const policy = definePolicy({
  ...
  permissions: {
    admin:  { post: ['*'] },
    member: { post: { read: true, update: { when: 'isAuthor' } } },
  },
});
...
// Express needs 4 params for it to recognise this as an error handler.
